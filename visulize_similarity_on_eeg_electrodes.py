...
DTW_result = {}
data = pd.read_excel('datasets/incoungerunet_subject1.xls')
...
for i in data:
    for j in data:
        if j == i:
            continue
        DTW_result[np.around(dis.dtw.distance_fast(arr.array('d', data[i]), arr.array('d', data[j])), decimals=3)] = (i, j)



edgeslabel = dict([[v, k] for k, v in DTW_result.items()])
electrodes = []
# print(data_matrix)
for i in data:
    electrodes.append(i)
